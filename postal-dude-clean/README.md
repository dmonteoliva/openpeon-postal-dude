# Postal Dude (POSTAL 2) - Clean

The Postal Dude from POSTAL 2 reacts to your coding session. Profanity-free cut.

This is the **profanity-free** cut. For the uncensored pack, see [`postal-dude`](../postal-dude/).
...
```bash
peon packs install postal-dude-clean
peon packs use postal-dude-clean
```
...
peon packs install-local openpeon-postal-dude/postal-dude-clean
peon packs use postal-dude-clean
```

## Contents

54 clips across all 9 CESP categories.

| Category | Clips | Fires when |
|---|---:|---|
| `session.start` | 6 | CLI launches / new session begins |
| `task.acknowledge` | 7 | Agent accepted work, processing started |
| `task.complete` | 7 | Work finished successfully |
| `task.error` | 5 | Something failed |
| `input.required` | 6 | Blocked waiting for user input or approval |
| `resource.limit` | 6 | Rate limit, token limit, or quota hit |
| `task.progress` | 7 | Long-running task heartbeat |
| `session.end` | 4 | Session closes gracefully |
